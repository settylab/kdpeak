    ...
    def test_kdpeak_parameter_validation(self):
        ...
        try:
            ...
            result = subprocess.run([
                'kdpeak', test_bed,
                '--out', '/tmp/test.bed',
                '--kde-bw', '-100'  # Negative bandwidth should be invalid
            ], capture_output=True, text=True, timeout=10)
...
class TestBWOpsCLI:
    ...
    def test_bwops_subcommand_help(self):
        ...
        for subcommand in ['add', 'multiply', 'regress']:
            result = subprocess.run(['bwops', subcommand, '--help'], 
...
class TestCLIErrorHandling:
    ...
    def test_kdpeak_invalid_chrom_sizes(self, small_bed_file):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run([
                'kdpeak', str(small_bed_file),
                '--out', os.path.join(output_dir, 'peaks.bed'),
                '--density-out', os.path.join(output_dir, 'density.bw'),
                '--chrom-sizes', '/nonexistent/sizes.txt'
