...
class TestKdpeakCLIErrorHandling:
    ...
    def test_kdpeak_permission_denied_output(self):
        ...
        try:
            ...
            protected_output = "/root/test_output.bed" if os.path.exists("/root") else "/dev/null/test_output.bed"
            
            result = subprocess.run([
                sys.executable, '-m', 'kdpeak.core',
                bed_path,
                '--out', protected_output
            ], capture_output=True, text=True)
    ...
    def test_kdpeak_help_displays_correctly(self):
        ...
        assert "kdpeak" in result.stdout or "kernel density estimation" in result.stdout.lower()
...
class TestBwopsCLIErrorHandling:
    ...
    def test_bwops_add_missing_files(self):
        ...
        result = subprocess.run([
            sys.executable, '-m', 'kdpeak.bwops', 'add',
            'nonexistent1.bw', 'nonexistent2.bw',
            '--out', 'output.bw'
        ], capture_output=True, text=True)
    ...
    def test_bwops_add_missing_chrom_sizes(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.bw', delete=False) as bw1, \
             tempfile.NamedTemporaryFile(suffix='.bw', delete=False) as bw2:
    ...
    def test_bwops_regress_invalid_formula(self):
        ...
        result = subprocess.run([
            sys.executable, '-m', 'kdpeak.bwops', 'regress',
    ...
    def test_bwops_correlate_insufficient_files(self):
        ...
        result = subprocess.run([
            sys.executable, '-m', 'kdpeak.bwops', 'correlate',
...
class TestCLIExitCodes:
    ...
    def test_argument_error_exit_codes(self):
        ...
        commands = [
            ...
            [sys.executable, '-m', 'kdpeak.bwops', 'invalid_operation'],  # Invalid choice
        ]
...
class TestCLIErrorMessageQuality:
    ...
    def test_error_messages_are_user_friendly(self):
        ...
        assert any(word in error_output.lower() for word in ["check", "verify", "ensure"])
