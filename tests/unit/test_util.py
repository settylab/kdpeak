...
class TestEventsProcessing:
    ...
    def test_events_from_intervals(self):
        ...
        intervals = pd.DataFrame(
            {"seqname": ["chr1", "chr1"], "start": [1000, 2000], "end": [1500, 2500]}
        )
...
class TestChromosomeHandling:
    ...
    def test_get_chromosome_size_estimate(self):
        ...
        events_df = pd.DataFrame({"location": [1000, 5000, 10000, 25000]})
    ...
    def test_filter_chromosomes_both_filters(self):
        ...
        filtered = filter_chromosomes(
            chromosomes, exclude_contigs=True, chromosome_pattern=r"chr[1-9XY]+"
        )
...
class TestSortChromosomes:
    ...
    def test_sort_chromosomes_by_size_with_file(
        self, sample_events_dict, chrom_sizes_file
    ):
        """Test sorting using actual chromosome sizes."""
        sorted_chroms = sort_chromosomes_by_size(
            sample_events_dict, chrom_sizes_file=str(chrom_sizes_file)
        )
    ...
    def test_sort_chromosomes_with_filtering(self, sample_events_dict):
        ...
        sample_events_dict["chr1_random"] = pd.DataFrame({"location": [1000, 2000]})

        sorted_chroms = sort_chromosomes_by_size(
            sample_events_dict, exclude_contigs=True
        )
...
class TestKDEFunctions:
    ...
    def test_make_kdes_basic(self, sample_events_dict):
        ...
        comb_data, signal_list = make_kdes(sample_events_dict, step=100, kde_bw=500)
    ...
    def test_make_kdes_with_blacklist(self, sample_events_dict):
        ...
        comb_data, signal_list = make_kdes(sample_events_dict, blacklisted=["chrX"])
    ...
    def test_make_kdes_with_filtering(self, sample_events_dict):
        ...
        sample_events_dict["chr1_random"] = pd.DataFrame(
            {"variable": ["start"] * 10, "location": np.random.randint(1000, 5000, 10)}
        )

        comb_data, signal_list = make_kdes(sample_events_dict, exclude_contigs=True)
