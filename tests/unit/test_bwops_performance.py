...
class TestBigWigFastReading:
    ...
    def temp_bigwig_100bp(self):
        ...
        with pyBigWig.open(temp_file.name, "w") as bw:
            ...
            values = [
                float(i + 10) for i in range(len(starts))
    ...
    def test_native_resolution_detection(self, temp_bigwig_100bp, temp_bigwig_50bp):
        ...
        resolution, needs_warning = get_native_resolution(
            [temp_bigwig_100bp, temp_bigwig_50bp], ["chr1"]
        )
    ...
    def test_stats_reading_when_spans_dont_match(self, temp_bigwig_100bp):
        ...
        assert (
            len(data) == 5
        )  # chr1 with 200bp intervals: 0-200, 200-400, 400-600, 600-800, 800-1000
    ...
    def test_multiple_files_mixed_compatibility(
        self, temp_bigwig_100bp, temp_bigwig_50bp
    ):
        """Test behavior when some files can use fast reading and others cannot."""
        data = read_bigwig_data(
            [temp_bigwig_100bp, temp_bigwig_50bp], ["chr1"], span=100
        )
    ...
    def test_fast_reading_logging(self, mock_get_logger, temp_bigwig_100bp):
        ...
        assert (
            fast_reading_logged
        ), f"Fast reading not logged. Info calls: {info_calls}, Debug calls: {debug_calls}"
    ...
    def test_stats_reading_logging(self, mock_get_logger, temp_bigwig_100bp):
        ...
        assert (
            stats_reading_logged
        ), f"Stats reading not logged. Info calls: {info_calls}, Debug calls: {debug_calls}"
    ...
    def test_region_limiting(self, temp_bigwig_100bp):
        ...
        data = read_bigwig_data(
            [temp_bigwig_100bp], ["chr1"], span=100, region=("chr1", 0, 500)
        )
    ...
    def test_performance_warning_threshold(self):
        ...
        try:
            ...
            with patch("logging.getLogger") as mock_get_logger:
                ...
                read_bigwig_data(
                    [temp_file.name], ["chr1"], span=1
                )  # 1bp span = 10M intervals
                ...
                assert (
                    large_intervals_warned
                ), f"Large intervals warning not found. Warnings: {warning_calls}"
        finally:
            Path(temp_file.name).unlink()
...
class TestBigWigPerformanceIntegration:
    ...
    def temp_bigwig_files(self):
        ...
        for i in range(3):
            ...
            with pyBigWig.open(temp_file.name, "w") as bw:
                ...
                starts = list(range(0, 500, 100))  # 0, 100, 200, 300, 400
                values = [
                    float(i * 10 + j) for j in range(len(starts))
                ]  # File 0: 0,1,2,3,4; File 1: 10,11,12,13,14; etc.
                bw.addEntries("chr1", starts, values=values, span=100)
    ...
    def test_fast_reading_multiple_files(self, temp_bigwig_files):
        ...
        for i, col in enumerate(sorted(bw_cols)):
            ...
            ), f"File {i} values mismatch: expected {expected_values}, got {actual_values}"
    ...
    def test_consistency_between_fast_and_stats_reading(self, temp_bigwig_files):
        ...
        for col in bw_cols:
            pd.testing.assert_series_equal(
                fast_data[col], stats_data[col], check_names=False
            )
