...
class TestEdgeCases:
    ...
    def test_very_large_coordinates(self):
        ...
        events_dict = {
            'chr1': pd.DataFrame({
                'variable': ['start'] * len(large_coords),
                'location': large_coords
            })
        }
    ...
    def test_many_small_chromosomes(self):
        ...
        for i in range(100):
            ...
            events_dict[scaffold_name] = pd.DataFrame({
                'variable': ['start'] * 5,
                'location': np.random.randint(0, 10000, 5)
            })
        ...
        assert processing_time < 30, f"Too slow with many chromosomes: {processing_time:.1f}s"
    ...
    def test_extreme_parameters(self, sample_events_dict):
        ...
        comb_data_highres, _ = make_kdes(
            sample_events_dict,
            kde_bw=200,
            step=1  # Single base pair resolution
        )
