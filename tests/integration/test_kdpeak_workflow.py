...
class TestKDPeakWorkflow:
    ...
    def test_workflow_with_bigwig_output(self, small_bed_file, chrom_sizes_file, temp_dir):
...
class TestWorkflowComponents:
    ...
    def test_filtering_pipeline(self, sample_events_dict, chrom_sizes_file):
        ...
        sample_events_dict['chr1_random'] = pd.DataFrame({
            'variable': ['start'] * 5,
            'location': [1000, 2000, 3000, 4000, 5000]
        })
        sample_events_dict['chrUn_scaffold123'] = pd.DataFrame({
            'variable': ['start'] * 3,
            'location': [500, 1500, 2500]
        })
    ...
    def test_parameter_sensitivity(self, sample_events_dict):
        ...
        for frip in [0.1, 0.3, 0.5]:
            peaks = call_peaks(
                comb_data, 
                signal_list, 
                fraction_in_peaks=frip,
                span=base_params['step']
            )
...
class TestRealWorldScenarios:
    ...
    def test_multi_species_chromosomes(self):
        ...
        species_data = {
            # Human-style
            'chr1': pd.DataFrame({'variable': ['start']*10, 'location': range(1000, 11000, 1000)}),
            'chrX': pd.DataFrame({'variable': ['start']*5, 'location': range(2000, 7000, 1000)}),
            
            # Mouse-style (same as human in this case)
            'chr19': pd.DataFrame({'variable': ['start']*8, 'location': range(3000, 11000, 1000)}),
            
            # Drosophila-style
            'chr2L': pd.DataFrame({'variable': ['start']*6, 'location': range(1500, 7500, 1000)}),
            'chr3R': pd.DataFrame({'variable': ['start']*7, 'location': range(2500, 9500, 1000)}),
            
            # Should be filtered out
            'chr2L_random': pd.DataFrame({'variable': ['start']*3, 'location': [500, 1500, 2500]})
        }
        ...
        comb_data, signal_list = make_kdes(
            species_data,
            exclude_contigs=True,
            chromosome_pattern=r'chr[0-9XY]+[LR]?'
        )
